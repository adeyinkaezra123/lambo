...
export const SceneManager = (() => {
  ...
  const loadStage = (scene) => {
    ...
    if (!glTFLoader) throw new Error("SceneManager init should be called.");
  ...
  const loadActiveModel = (scene, meta) => {
    ...
    const dfCol_Body = ColorUtils.webColorToHex(
      meta.body_colors.colors[defBody].value
    );
    const dfCol_Mirror = ColorUtils.webColorToHex(
      meta.mirror_colors.colors[defMirror].value
    );
    const dfCol_Alloys = ColorUtils.webColorToHex(
      meta.wheel_colors.colors[defAlloys].value
    );
    const dfCol_Caliper = ColorUtils.webColorToHex(
      meta.caliper_colors.colors[defCaliper].value
    );
    ...
    glTFLoader.setPath(ACTIVE_PATH).load("model.glb", (model) => {
      ...
      mActiveModel.traverse((obj) => {
        ...
        if (obj instanceof Mesh) {
          //Set Color - Body
          if (obj.material.name === "Mt_Body") obj.material.color = dfCol_Body;
        ...
        if (
          obj.name.includes("Obj_Rim") &&
          !obj.name.includes(meta.wheel_designs.designs[0].value)
        )
          obj.visible = false;
      });
